      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <schxslt:rule pattern="d7e328">
               ...
               <xsl:if test="boolean(//*[matches(@name, '01\d{5}')])">
                  <xsl:if test="not(exists(./cac:AccountingCustomerParty/cac:Party/cac:PartyTaxScheme/ cbc:CompanyID))">
                     <xsl:if test="not(exists(./cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID) and normalize-space(./cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID) !='' )">
