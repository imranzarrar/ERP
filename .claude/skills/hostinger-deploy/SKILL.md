...
- **Every VPS-side command block you hand the user starts with `su - deploy`** (explicitly requested by the user as a standing habit) — never just assume they're already on that user. PM2 keeps a completely separate process list per OS user, so a command run as the wrong one silently does nothing useful (see the reboot incident below). The one exception is the handful of steps that genuinely must run as root the very first time (initial SSH hardening, before the `deploy` user's own access is confirmed working) — call those out explicitly as staying on root instead.
...
- Real incident, now fixed: **`deploy/backup-db.sh` failed `pg_dump` password auth on a password the app itself connects with just fine.** Root cause: the script parsed `app.secrets` with raw bash `grep`/`cut`, which doesn't strip a trailing `\r` (Windows line ending) or surrounding quotes the way Node's `dotenv` (what the app actually uses to read the same file) does — so a value written either way reached `pg_dump` with extra characters the app never sees. Fixed in the script itself (it now strips both the same way dotenv does) — if this class of "script X can't connect but the app can" ever recurs for a *different* script that hand-parses `app.secrets` in bash, suspect this exact gap first before assuming the credential itself is wrong.
...
- `deploy/setup-rls.sh` / `deploy/verify-rls.sh` — one-time creation of the restricted `erp_app_tenant` role and the post-deploy RLS check. Every deploy that runs `db:push` must be followed by `npx tsx scripts/apply-rls-policies.mjs` (drizzle-kit drops policy conditions) and `verify-rls.sh`. `TENANT_DB_USER`/`TENANT_DB_PASSWORD` must be in `app.secrets` (letters+digits password).
- `deploy/nginx-tuning.sh` (+ `deploy/nginx-snippets/`) — HTTP/2, gzip and immutable `/assets/` caching; part of provisioning any new host, see docs/deployment-plan.md Part 3.
