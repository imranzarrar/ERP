...
- **Browser-pane visibility/compositing issue is recurring, not a one-off (items 31, 33)**: `document.hidden` intermittently stays `true` across separate sessions (confirmed fine mid-way through item 32's session — real screenshots obtained then — but stuck again in item 33's session, including after a `requestAnimationFrame`→`setTimeout` monkey-patch attempt to force it). This blocks live click-through verification of any Framer Motion `AnimatePresence`-gated view whenever it happens, tied to whether the Browser pane panel is actually visible/focused on the user's end at that moment — not something fixable from code or from within the page. When it's blocking, code-level verification (tsc, direct bundle inspection, DB checks) is the fallback, but a manual click-through should still happen once the pane is genuinely visible.
- **POS never reaches the real ZATCA pipeline (item 32)**: `PosModule.tsx` still saves through the legacy client-side `saveInvoice()`/`/api/migrate` blob path, not the dedicated `/api/transactions/invoices` route — confirmed live that a completed POS sale has empty `invoice_items` and `zatcaStatus` stuck at `NOT_SUBMITTED` forever. Deliberately left out of scope by explicit project-owner instruction during item 32's push, but flagged as a near-term priority given POS is a live, real-money sales channel with zero ZATCA compliance today.
...
- **Backend `dev` script has no watch/auto-restart (item 35)**: `tsx server.ts` only picks up backend changes on a manual restart — only the Vite middleware's client bundle hot-reloads automatically. A real risk of silently testing against stale server code (as happened during item 35 itself). `tsx watch server.ts` would fix this; not changed yet since it's a dev-workflow change outside what was asked.
...
- **Recurring Expenses has no dedicated backend persistence endpoints (item 32's Foundation fix)**: template/accrual CRUD still goes through the generic `handleUpdateDb`/`/api/migrate` blob-sync path rather than real per-action REST routes (contrast with Invoices/Expenses/Inventory, which all got dedicated routes this session). The Foundation fix made failures on this path visible (`cloudError` now renders), but the underlying architecture gap — no real endpoint, no transactional guarantee — is unresolved.
