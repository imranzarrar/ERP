...
// Customer Master / Vendor Master bulk import — same Validate-then-Commit shape as
// ProductImportModal.tsx, parameterized by `entity` since the two masters share almost
// every field. See server/lib/partyImport.ts for the matching rules (VAT is authoritative;
// phone is only a fallback, and a phone match against a DIFFERENT party than the VAT points
// to is a conflict, never a silent update).
...
export default function PartyImportModal({ db, entity, onClose, onImported }: {
  db: DatabaseState; entity: 'customer' | 'vendor'; onClose: () => void; onImported: () => Promise<void> | void;
}) {
  ...
  const handleDownloadTemplate = async () => {
    const res = await fetch(`/api/master-import/${base}/template`);
    if (!res.ok) { setError(t('Could not download the template.')); return; }
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `${base}-import-template.xlsx`;
  ...
  const handleFileChosen = async (f: File) => {
    ...
    try {
      ...
      fd.append('file', f);
      const res = await fetch(`/api/master-import/${base}/validate`, { method: 'POST', body: fd });
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/master-import/${base}/commit`, { method: 'POST', body: fd });
      const body = await res.json().catch(() => ({}));
                          ...
                          <tr key={r.rowNumber} className={r.action === 'error' ? 'bg-rose-50/40' : excluded.has(r.rowNumber) ? 'opacity-40' : ''}>
                            ...
                            <td className="p-2">
                              {r.action === 'error' ? (
                                ...
                                <span className="text-indigo-600 font-bold">{t('Update existing (VAT match)')}</span>
                              ) : (
                                <span className="text-emerald-600 font-bold">{entity === 'customer' ? t('New customer') : t('New vendor')}</span>
                              )}
                            </td>
                          </tr>
                        ))}
