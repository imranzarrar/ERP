...
async function makeCompany(name: string, hrSettings: any = null) {
  ...
  await db.insert(schema.companies).values({
    id, name, address: 'x', phone: '0', email: `${id.slice(0, 8)}@example.com`,
...
describe('POST /users — employeeId mandatory once the company has adopted HR onboarding', () => {
  it('allows a new account with no employeeId when the company has zero employees', async () => {
    const { status, body } = await api(emptyAdminSessionId, '/api/users', {
      method: 'POST',
      body: JSON.stringify({ id: generateId(), username: `nohr_${generateId().slice(0, 8)}`, email: 'nohr@example.com', password: 'x', role: 'admin', companyId: emptyCompanyId }),
    });
    expect(status).toBe(200);
  ...
  it('rejects a new account with no employeeId once the company has an active employee', async () => {
    const { status, body } = await api(adminSessionId, '/api/users', {
      method: 'POST',
      body: JSON.stringify({ id: generateId(), username: `needshr_${generateId().slice(0, 8)}`, email: 'needshr@example.com', password: 'x', role: 'admin', companyId }),
    });
    expect(status).toBe(400);
  ...
  it('rejects linking a deactivated employee to a new account', async () => {
    ...
    const { status, body } = await api(adminSessionId, '/api/users', {
      method: 'POST',
      body: JSON.stringify({ id: generateId(), username: `deactemp_${generateId().slice(0, 8)}`, email: 'deactemp@example.com', password: 'x', role: 'admin', companyId, employeeId: deactivatedRes.body.id }),
    });
    expect(status).toBe(400);
    expect(body.error).toMatch(/deactivated/i);
  });
  ...
  it('accepts a new account linked to a valid active employee', async () => {
    ...
    const { status, body } = await api(adminSessionId, '/api/users', {
      method: 'POST',
      body: JSON.stringify({ id: newUserId, username: `withhr_${generateId().slice(0, 8)}`, email: 'withhr@example.com', password: 'x', role: 'admin', companyId, employeeId: employeeAId }),
    });
    expect(status).toBe(200);
